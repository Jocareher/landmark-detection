...
def run_epoch(
    ...
    pca_diagnostic_meters = {key: AverageMeter() for key in PCA_DIAGNOSTICS}
    ...
    for batch in progress_bar:
        ...
        for key, meter in pca_diagnostic_meters.items():
            meter.update(loss_dict[key].item(), batch_size)
    ...
    return {
        ...
        **{key: meter.avg for key, meter in pca_diagnostic_meters.items()},
        "nme": nme_meter.avg,
...
def append_results_row(
    ...
    csv_path = Path(csv_path)
    # Match an existing header when appending to a CSV from an older run.
    with csv_path.open("r", newline="", encoding="utf-8") as file:
        fieldnames = next(csv.reader(file))
    row = {
        **metrics, "epoch": epoch, "split": split, "lr": lr,
        "landmark_loss": landmark_loss, "coordinate_decoder": coordinate_decoder,
    }
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        if use_wandb and wandb is not None:
            wandb.log(
                {
                    **{f"train/{key}": train_metrics[key] for key in PCA_DIAGNOSTICS},
                    **{f"val/{key}": val_metrics[key] for key in PCA_DIAGNOSTICS},
                    "epoch": epoch,
                    "lr": current_lr,
